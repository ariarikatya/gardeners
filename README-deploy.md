...
- Требования и особенности iOS (Safari):
  - Для работы Web Push на iOS (iPhone/iPad) устройство должно быть сохранено как PWA: нажмите кнопку «Поделиться» (Share) → «На экран „Домой“» («Add to Home Screen»).
  - Запустите приложение с иконки на рабочем столе и включите уведомления.

...
async function updateLeadStage(leadId, serviceName, action) {
  ...
  let stageId = await findStageId(mapping.pipeline, mapping.stage, action);

  if (!stageId) {
    // Fallback ID based on pipeline and action
    const lowerPipe = mapping.pipeline.toLowerCase();
    if (lowerPipe.includes('агро 2026')) {
      stageId = action === 'refusal' ? 142 : 143; // На проверку или Выполнено
    } else if (lowerPipe.includes('сервис') || lowerPipe.includes('обрезка')) {
      stageId = action === 'refusal' ? 'Отказные' : 'Выполнено';
    }
  }

  // Пробуем обычный PATCH
  const payload = [{ id: Number(leadId), status_id: stageId }];
  try {
    const result = await apiRequest('/api/v4/leads', { method: 'PATCH', body: JSON.stringify(payload) });
    console.log('✅ PATCH успех');
    return result;
  } catch (e) {
    console.log('️ PATCH не сработал, пробуем leads/links');
    // Если PATCH не сработал, используем leads/links
    const linkPayload = {
      links: [{
        from: {
          entity_id: Number(leadId),
        ...
        to: {
          pipeline_id: mapping.pipeline.includes('АГРО') ? 9104942 : mapping.pipeline.includes('СЕРВИС') ? 4032988 : 7494438,
          status_id: stageId
        }
      }]
    };
    return await apiRequest('/api/v4/leads/links', { method: 'POST', body: JSON.stringify(linkPayload) });
  }
}

...
async function findStageId(pipelineName, stageName, action = null) {
  ...
  if (lowerPipeName.includes('агро 2026') || lowerPipeName.includes('агро')) {
    if (action === 'refusal' || lowerStage.includes('отказ')) {
      fallbackStatusId = 143; // Отказные / Отказ в Агро 2026
    } else if (action === 'complete' || lowerStage.includes('проверку')) {
      fallbackStatusId = 142; // Успешно / На проверку в Агро 2026
    }
  }
...
function determinePipelineAndStageByService(serviceName, intent = 'initial', statusAction = null) {
  ...
  if (isObrezka) {
    ...
    if (statusAction === 'complete') return { pipeline: '2024 ОБРЕЗКА', stage: 'Выполнено' };
    return { pipeline: '2024 ОБРЕЗКА', stage: 'Назначен сотрудник' };
  }

  if (isService) {
    if (statusAction === 'refusal') return { pipeline: '2024 СЕРВИС', stage: 'Отказные' };
    if (statusAction === 'complete') return { pipeline: '2024 СЕРВИС', stage: 'Выполнено' };

...
async function getRefreshTokenFromDbOrEnv() {
  ...
  return process.env.AMO_REFRESH_TOKEN;
}
...
async function saveRefreshTokenToDb(token) {
  ...
  try {
    await prisma.systemSetting.upsert({
      ...
      update: { value: token },
      create: { key: 'AMO_REFRESH_TOKEN', value: token },
    });
  } catch (e) {
...
async function getAccessToken() {
  ...
  const currentRefreshToken = await getRefreshTokenFromDbOrEnv();
...
function determinePipelineAndStageByService(serviceName, intent = 'initial', statusAction = null) {
  ...
  // default 2024 Агро
  if (statusAction === 'refusal' || statusAction === 'complete') return { pipeline: '2024 Агро', stage: 'на проверку' };
  return { pipeline: '2024 Агро', stage: 'новый заказ' };
}
...
async function createLead({ name, phone, note, serviceName }) {
  ...
  try {
    ...
    console.log('🎯 Ищу воронку и этап:', mapping);

    const stageId = await findStageId(mapping.pipeline, mapping.stage);
    if (!stageId) {
      console.warn('⚠️ Не нашел этап...', mapping);
    }

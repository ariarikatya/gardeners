...
function httpRequest(urlStr, options = {}, bodyBuffer = null, idleTimeoutMs = 15000) {
  return new Promise((resolve, reject) => {
    ...
    const resetTimer = () => {
      ...
      timer = setTimeout(() => {
        if (isSettled) return;
        isSettled = true;
        const err = new Error(`Socket idle timeout after ${idleTimeoutMs}ms`);
        req.destroy(err);
        reject(err);
      }, idleTimeoutMs);
    };
...
async function requestWithRetry(url, options = {}, bodyBuffer = null, config = {}) {
  ...
  const idleTimeoutMs = config.timeoutMs ?? 15000;
  const retryDelays = config.retryDelays ?? [500, 500];
  ...
  while (attempt < maxAttempts) {
    ...
    const startTime = Date.now();
    try {
      const res = await httpRequest(url, options, bodyBuffer, idleTimeoutMs);
  ...
  for (const seg of segments) {
    ...
    try {
      const res = await requestWithRetry(
        ...
        null,
        { maxAttempts: 2, timeoutMs: 15000, retryDelays: [500, 500] }
      );

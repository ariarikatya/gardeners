...
export default function AdminUsersPage() {
  ...
  const handleGardenerPhotoUpload = async (gardenerId, file) => {
    ...
    reader.onloadend = async () => {
      ...
      const uploadRes = await fetch('/api/upload-image', { method: 'POST', body: JSON.stringify({ image: base64 }) });
      const { url } = await uploadRes.json();
      await fetch(`/api/admin/gardeners`, { method: 'PUT', body: JSON.stringify({ id: gardenerId, photo: url }) });
  ...
  const handleGardenerUpdate = async (gardenerId, field, value) => {
    await fetch(`/api/admin/gardeners`, { method: 'PUT', body: JSON.stringify({ id: gardenerId, [field]: value }) });
    alert('Сохранено');
  };
